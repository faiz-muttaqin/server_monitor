...
func SanitizePhoneNumber(phone string) (string, error) {
	...
	if !IsValidPhoneNumber(phone) {
		return "", fmt.Errorf("invalid phone number: %v Nomor Telepon Bukan dari Wilayah Indonesia", phone)
	}
	// Validate phone number length (excluding leading '0')
	phoneWithoutZero := phone[1:]
	if len(phoneWithoutZero) < 9 || len(phoneWithoutZero) > 13 {
		return "", fmt.Errorf("invalid phone number length: %v (should be between 9 and 13 digits after '0')", phoneWithoutZero)
	}

	// Return the sanitized phone number without the leading '0'
	return phoneWithoutZero, nil
}
