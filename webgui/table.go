...
func Table(title, table_name, endpoint string, page_length int, length_menu []int, order []any, table_columns []Column, insertable, editable, deletable, hideHeader, passwordable bool, scrollUpDown, scrollLeftRight bool, exportType []string) template.HTML {
	...
	for i, col := range table_columns {
		...
		case "string":
			...
			table_columns[i].ClassName = className
			table_columns[i].Targets = i
			table_columns[i].Render = template.HTML(returnValue)
		...
		case "image":
			...
			table_columns[i].ClassName = className
			table_columns[i].Targets = i
			table_columns[i].Render = template.HTML(fmt.Sprintf(`<div style="width: 50px;height: 50px;overflow: hidden;">%s</div>`, returnValue))
		...
		if table_columns[i].Filterable {
			filterable = true
		}
	}
	// fmt.Println("filterable")
	// fmt.Println(filterable)
	actionable := ""
