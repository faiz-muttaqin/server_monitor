...
$(document).ready(function () {
	...
	function loadAndShowTab(targetTabId) {
		...
		if (targetTabId.indexOf("#") >= 0) {
			console.log("Loading tab index of #:", targetTabId);
			// console.log(targetTabId);
			if ($(targetTabId).children().length > 0) {
				console.log("Tab already loaded:", targetTabId);
				// $(targetTabId).show();
				...
				if (isTabValid) {
					console.log("Tab is valid, fetching content:", targetTabId);
	...
	// Check if the name element exists before proceeding
	if ($("#user-admin-name").length) {
