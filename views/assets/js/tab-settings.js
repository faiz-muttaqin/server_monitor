        ...
        columnDefs: [
          ...
          },
          {
            ...
            render: function (data, type, full, meta) {
              var btn = ""
              if (full["start"] == "1" ){
                btn = `<button type="button" onclick="restartService('${data}','${full["unit"]}')" class="btn btn-sm btn-outline-info p-1" title="Restart"><span class="tf-icons bx bx-power-off bx-flashing-hover"></span></button>`;
              }
              return btn;
            }
          },
...
function restartService(data, unitname) {
  // Show a prompt for username and password
  Swal.fire({
    title: 'Restart Service',
    html: `
      <p>Do you really want to restart the ${unitname} service?</p>
    ...
    if (result.isConfirmed) {
      ...
      // Add your fetch logic here, passing username and password as needed
      fetch(webPath + `tab-settings/systems/services/restart?s=${data}`, {
        ...
        .catch(error => {
          ...
          Swal.fire(
            'Error!',
            error.error, // Use the specific error message from the backend
            'error'
          );
        });
    }
