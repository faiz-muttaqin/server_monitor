      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            const parts = meta.settings.ajax.split('/');
            return ('<p class="'+editable+' mb-0" patch="tab-roles/'+parts[parts.length - 2]+'" field="phone" point="'+full['id']+'" >'+full['phone']+'</p>');
          }
        },
        {
          // User Role
          ...
          render: function (data, type, full, meta) {
            ...
            const parts = meta.settings.ajax.split('/');
            return (`<p class="${classname} ${selectablechoice}" tab="${meta.settings.sTableId}" patch="tab-roles/${parts[parts.length - 2]}" field="role" point="${full['id']}" origin="${title}" choices="${choices}">${title}</p>`)
          }
        },
        {
          // User Status
          ...
          render: function (data, type, full, meta) {
            ...
            const parts = meta.settings.ajax.split('/');
            return (`<p class="${classname} ${selectablechoice}" tab="${meta.settings.sTableId}" patch="tab-roles/${parts[parts.length - 2]}" field="status" point="${full['id']}" origin="${title}" choices="${choices}">${title}</p>`)
          }
        },
        {
          // Actions
              ...
              '<div class="d-inline-block text-nowrap">' +
                '<div class="dropdown">' +
                  ...
                  '<div class="dropdown-menu dropdown-menu-end" aria-labelledby="actionsDropdown">' +
                    '<button class="btn dropdown-item text-danger deleteable" tab="'+tableId+'" delete="tab-roles/'+parts[parts.length - 2]+'/'+full['id']+'">Remove</button>' +
                  '</div>' +
                '</div>' +
              '</div>'
              );
